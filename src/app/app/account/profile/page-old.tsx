...
export default function ProfilPage() {
  ...
  const getInstruments = (instruments?: string) => {
    if (!instruments) return ['Musicien'];
    try {
  ...
  // Données du profil utilisateur basées sur la session
  const [userProfile, setUserProfile] = useState({
    id: session?.user?.id || '1',
    name: session?.user?.name || 'Utilisateur',
    email: session?.user?.email || 'email@exemple.com',
    phone: '+33 6 12 34 56 78', // À récupérer depuis la DB
    address: '123 Rue de la Paix, 75001 Paris', // À récupérer depuis la DB
    avatar: session?.user?.name ? getInitials(session.user.name) : 'U',
    role: session?.user?.role || 'MUSICIEN',
    status: 'disponible',
    instruments: session?.user?.instruments ? getInstruments(session.user.instruments) : ['Musicien'],
    experience: 8, // À récupérer depuis la DB
    joinedDate: '2020-03-15', // À récupérer depuis la DB
    lastActive: '2024-01-12', // À calculer depuis la session
    bio: 'Musicien passionné au service de l\'Église ACER PARIS', // À récupérer depuis la DB
    specialties: ['Louange', 'Accompagnement'], // À récupérer depuis la DB
    rating: 4.8, // À calculer depuis la DB
    servicesCount: 45, // À récupérer depuis la DB
    uploadsCount: 12, // À récupérer depuis la DB
    availability: {
      sunday: true,
      weekdays: false,
      rehearsals: true,
